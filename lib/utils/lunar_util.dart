...
class LunarUtil {
  ...
  static List<int> _solarFromInt(int g) {
    int y = ((10000 * g + 14780) / 3652425) .toInt();
    int ddd = g - (365 * y + (y / 4).toInt() - (y / 100).toInt() + (y / 400).toInt());
    if (ddd < 0) {
      y--;
      ddd = g - (365 * y + (y / 4).toInt() - (y / 100).toInt() +(y / 400).toInt());
    }
    int mi = ((100 * ddd + 52) / 3060) .toInt();
    int mm = (mi + 2) % 12 + 1;
    y = y + ((mi + 2) / 12) .toInt();
    int dd = ddd - ((mi * 306 + 5) / 10).toInt() + 1;
    List<int> solar = List(4);
    solar[0] = y;
    solar[1] = mm;
    solar[2] = dd;
    return solar;
  ...
  static List<int> lunarToSolar(int lunarYear, int lunarMonth, int lunarDay, bool isLeap) {
    int days = _LUNAR_MONTH_DAYS[lunarYear - _LUNAR_MONTH_DAYS[0]];
    int leap = _getBitInt(days, 4, 13);
    int offset = 0;
    int loop = leap;
    if (!isLeap) {
    ...
    int solar11 = _SOLAR[lunarYear - _SOLAR[0]];
